...
define("tinymce/Formatter", [
	...
	return function(ed) {
		...
		function defaultFormats() {
			register({
				...
				link: {inline: 'a', selector: 'a', remove: 'all', split: true, deep: true,
					...
					onformat: function(elm, CQAt, vars) {
		...
		function apply(name, vars, node) {
			...
			function setElementFormat(elm, CQAt) {
				CQAt = CQAt || format;

				if (elm) {
					if (CQAt.onformat) {
						CQAt.onformat(elm, CQAt, vars, node);
					}

					each(CQAt.styles, function(value, name) {
						dom.setStyle(elm, name, replaceVars(value, vars));
					});

					each(CQAt.attributes, function(value, name) {
						dom.setAttrib(elm, name, replaceVars(value, vars));
					});

					each(CQAt.classes, function(value) {
		...
		function toggle(name, vars, node) {
			...
			if (match(name, vars, node) && (!('toggle' in CQAt[0]) || CQAt[0].toggle)) {
...
define("tinymce/ui/FormatControls", [
	...
	function registerControls(editor) {
		...
		function toggleFormat(CQAt) {
			if (CQAt.control) {
				CQAt = CQAt.control.value();
			}

			if (CQAt) {
				editor.execCommand('mceToggleFormat', false, CQAt);
			}
		}
